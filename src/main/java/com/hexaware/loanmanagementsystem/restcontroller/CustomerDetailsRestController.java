...
public class CustomerDetailsRestController {
	...
	@GetMapping(value = "getCustomerName/{name}", produces = "application/json")
	@PreAuthorize("hasAnyAuthority('ROLE_USER')")
	public String getNameByUserName(@PathVariable String name) {
		
		
		return customerDetailsService.getNameByUserName(name);
	}
